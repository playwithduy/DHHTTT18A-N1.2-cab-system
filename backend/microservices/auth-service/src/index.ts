...
app.use((req, res, next) => {
  console.log(`\x1b[36m[AUTH_REQUEST]\x1b[0m ${req.method} ${req.path}`);
  next();
});
...
app.use((req, res, next) => {
  ...
  if (req.headers['x-gateway-secret'] !== GATEWAY_SECRET) {
    console.log('\x1b[31m%s\x1b[0m', `[ZERO-TRUST] Rejection: Missing or invalid x-gateway-secret from ${req.ip}`);
